...
```{r}
pathways <- behavior_sheet_cln |>
  #Create new variables that account for the pathway's proportion
  mutate(
    'Nesting Male Path' = sum(female_visits_by_nm)/sum(total_female_visits),
    'Satellite Male Path' = sum(female_visit_by_sat)/sum(total_female_visits),
    'Satellite Pass Path' = sum(female_visits_by_sat_passed_to_nm)/sum(total_female_visits),
    'Sneaker Male Path' = sum(female_visit_by_snk)/sum(total_female_visits)
  ) |>
  #Make the tibble only include these variables
  dplyr::select(`Nesting Male Path`:`Sneaker Male Path`) |>
  #Remove all the duplicates
  distinct() |>
  #Adjust the table to make the variables go into one coloumn
  pivot_longer(`Nesting Male Path`:`Sneaker Male Path`,
               names_to = 'Pathway',
  ...
  labs(x = 'Pathway', 
       ...
       title = "The Propotion of each Visitation Pathway for Females",) + #edits axis and title labels
  theme_classic() + #Sets the plot visuals
  scale_fill_manual(values = c('Nesting Male Path' = '#065143', 'Satellite Male Path' = '#db5a42', 'Satellite Pass Path' = '#7A9B76','Sneaker Male Path' = '#1481BA')) +
  theme(plot.title = element_text(hjust = 0.5)) + #Moves title to the middle
